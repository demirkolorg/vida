  ...
  globalSearch: async (data) => {
    try {
      ...
      const entitiesToSearch = entityTypes.length > 0 
        ? entityTypes.filter(et => availableEntities.includes(et))
        : availableEntities;

      // Her entity için arama yap
      for (const entityType of entitiesToSearch) {
        try {
          searchResults[entityType] = await service[`search${entityType.charAt(0).toUpperCase() + entityType.slice(1)}`](
            searchTerm, 
            statusClause, 
            limit, 
            includeRelations
    ...
    return await prisma.birim.findMany({
      where: {
        ...
        OR: [
          ...
          { aciklama: { contains: searchTerm, mode: 'insensitive' } }
        ]
      },
    ...
    return await prisma.personel.findMany({
      where: {
        ...
        OR: [
          ...
          { 
            AND: [
              { ad: { contains: searchTerm.split(' ')[0] || '', mode: 'insensitive' } },
              { soyad: { contains: searchTerm.split(' ')[1] || '', mode: 'insensitive' } }
            ]
          }
        ]
      },
    ...
    return await prisma.malzeme.findMany({
      where: {
        ...
        OR: [
          ...
          { aciklama: { contains: searchTerm, mode: 'insensitive' } },
          { kod: { contains: searchTerm, mode: 'insensitive' } },
          { bademSeriNo: { contains: searchTerm, mode: 'insensitive' } },
          { etmysSeriNo: { contains: searchTerm, mode: 'insensitive' } },
          { stokDemirbasNo: { contains: searchTerm, mode: 'insensitive' } }
        ]
      },
      include: includeRelations ? {
  ...
  searchMalzemeHareket: async (searchTerm, statusClause, limit, includeRelations) => {
    return await prisma.malzemeHareket.findMany({
      where: {
        ...
        OR: [
          { aciklama: { contains: searchTerm, mode: 'insensitive' } },
          { malzeme: { vidaNo: { contains: searchTerm, mode: 'insensitive' } } },
          { malzeme: { aciklama: { contains: searchTerm, mode: 'insensitive' } } }
        ]
      },
      include: includeRelations ? {
        malzeme: {
    ...
    try {
      ...
      const quickSearchPromises = [
        prisma.malzeme.findMany({
          ...
          take: limit
        }).then(results => { quickResults.malzeme = results; }),
        ...
        prisma.personel.findMany({
          ...
          take: limit
        }).then(results => { quickResults.personel = results; }),
        
        prisma.birim.findMany({
          where: {
            ...statusClause,
            ad: { contains: searchTerm, mode: 'insensitive' }
          },
          ...
          take: limit
        }).then(results => { quickResults.birim = results; })
      ];
