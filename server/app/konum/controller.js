...
const controller = {
  ...
  getByDepoId: async (req, res) => {
    const rota = 'getByDepoId';
    try {
      const data = req.body;

      if (!data.depoId) return response.error(req, res, HizmetName, rota, message.get.error, message.required.depoId);
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      if (!data.depoId) return response.error(req, res, HizmetName, rota, message.add.error, message.special.error.depoGerekli);
