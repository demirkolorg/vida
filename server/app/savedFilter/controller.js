...
const controller = {
  getAllByEntityType: async (req, res) => {
    ...
    try {
      ...
      if (!data.entityType) {
        return response.error(req, res, HizmetName, rota, message.list.error, 'EntityType zorunludur.');
      }
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      if (!data.filterName) {
        return response.error(req, res, HizmetName, rota, message.add.error, 'Filtre adı (filterName) zorunludur.');
      }
      if (!data.entityType) {
        return response.error(req, res, HizmetName, rota, message.add.error, 'Varlık türü (entityType) zorunludur.');
      }
      if (!data.filterState) {
        return response.error(req, res, HizmetName, rota, message.add.error, 'Filtre durumu (filterState) zorunludur.');
      }
  ...
  search: async (req, res) => {
    ...
    try {
      ...
      if (!data.searchTerm && !data.entityType) {
        return response.error(req, res, HizmetName, rota, message.search.error, 'Arama terimi (searchTerm) veya varlık tipi (entityType) zorunludur.');
      }
