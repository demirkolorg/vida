...
const renderDetails = itemData => (
  ...
  <div className="space-y-3 rounded-md border bg-muted/30 p-4">
    ...
    {typeof itemData.konumlar?.length === 'number' && (
      <div className="flex items-center justify-between text-sm">
        <span className="font-medium text-muted-foreground">Bağlı Şube Sayısı:</span>
        <Badge variant={itemData.konumlar.length > 0 ? 'destructive_muted' : 'outline'} className="text-xs">
          {itemData.konumlar.length}
        </Badge>
      </div>
    )}
   
    <p className="text-xs text-destructive pt-2">
      Not: Birime bağlı şubeler veya malzemeler varsa, bu {EntityHuman} kaydını silmeden önce bu bağlantıları kaldırmanız veya ilgili kayıtları silmeniz/devretmeniz gerekebilir. Mevcut sistem bu tür bir kontrolü otomatik yapmıyorsa, silme işlemi
      veritabanı kısıtlamaları nedeniyle başarısız olabilir.
    </p>
  </div>
);

export const Depo_DeleteSheet = props => {
