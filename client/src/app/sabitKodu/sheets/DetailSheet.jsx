...
const renderDetails = itemData => {
  if (!itemData) {
    return <div className="p-4 text-center text-muted-foreground">SabitKodu bilgisi bulunamadı.</div>;
  }
    ...
    <ScrollArea className="h-[calc(100vh-230px)]">
      <div className="p-2 md:p-4">
        <DetailSection title="Temel Bilgiler">
          <DetailItem label="SabitKodu ID">
            <Badge variant="secondary" className="font-mono text-xs">
              {itemData.id}
            </Badge>
          </DetailItem>
          <DetailItem label="SabitKodu Adı">{itemData.ad || '-'}</DetailItem>
...
export const SabitKoduDetailSheet = props => {
  return (
