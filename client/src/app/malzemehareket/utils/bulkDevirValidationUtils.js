// src/app/malzemehareket/utils/bulkDevirValidationUtils.js

export function validateBulkDevirMalzemeler(malzemeler) {
  if (!malzemeler || malzemeler.length === 0) {
    return {
      isValid: false,
      errorType: 'EMPTY_LIST',
      errorMessage: 'Devredilecek malzeme listesi boş.',
      validPersonel: null,
      invalidPersoneller: [],
      validMalzemeler: [],
      invalidMalzemeler: []
  ...
  if (malzemelerWithoutPersonel.length > 0) {
    return {
      ...
      errorMessage: `${malzemelerWithoutPersonel.length} malzemenin zimmetli personel bilgisi bulunamadı. Bu malzemeler devir işlemi yapılamaz.`,
      validPersonel: null,
      invalidPersoneller: [],
      validMalzemeler: [],
  ...
  if (personelDataArray.length > 1) {
    ...
    return {
      ...
      errorMessage: `Seçilen malzemeler ${personelDataArray.length} farklı personelde zimmetli. Toplu devir işlemi için tüm malzemeler aynı personelde olmalıdır.`,
      validPersonel: validData.personel,
