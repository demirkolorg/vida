...
export const Personel_EditSheet = (props) => {
  ...
  const [passwordChangeEnabled, setPasswordChangeEnabled] = React.useState(
    false
  );
  ...
  const handleUpdateSubmit = async (id, formData) => {
    ...
    if (
      formData.sicil !== undefined &&
      formData.sicil !== currentItemForEdit?.sicil
    ) {
      payload.sicil = formData.sicil;
    }
    if (
      formData.role !== undefined &&
      formData.role !== currentItemForEdit?.role
    ) {
      payload.role = formData.role;
    }
    if (
      formData.avatar !== undefined &&
      formData.avatar !== currentItemForEdit?.avatar
    ) {
      payload.avatar = formData.avatar;
    }
    if (
      formData.buroId !== undefined &&
      formData.buroId !== currentItemForEdit?.buroId
    ) {
      payload.buroId = formData.buroId;
    }
    if (
      formData.isUser !== undefined &&
      formData.isUser !== currentItemForEdit?.isUser
    ) {
      payload.isUser = formData.isUser;
    }
    if (
      formData.isAmir !== undefined &&
      formData.isAmir !== currentItemForEdit?.isAmir
    ) {
    ...
    if (payload.parola) {
      toast.success(
        "Parola başarıyla değiştirildi. Yeni parola ile giriş yapabilirsiniz."
      );
    }
        ...
        {passwordChangeEnabled && (
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              ⚠️ Parola değiştirilirse, personel yeni parola ile giriş yapmak
              zorunda kalacaktır.
            </p>
          </div>
        )}
