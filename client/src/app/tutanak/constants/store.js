...
export const Tutanak_Store = createCrudStore(EntityHuman, EntityApiService, (set, get, baseStore) => {
  return {
    ...
    GetCurrentDatas: () => {
      const state = get();
      return state.selectedHareketTuru ? state.hareketTuruFilteredDatas : state.datas;
    },

    // Toplam tutanak sayısını döndürme
    GetTotalCount: () => {
      const state = get();
      return state.selectedHareketTuru ? state.hareketTuruFilteredDatas.length : state.datas.length;
    },

    // YENİ: Personel zimmet bilgi fişi oluştur
    GeneratePersonelZimmetBilgiFisi: async (personelId, options) => {
      const showSuccessToast = options?.showToast ?? true;
      if (get().loadingAction) return null;

      if (!personelId) {
        toast.error('Personel ID gerekli.');
        return null;
      }

      set({ loadingAction: true, error: null });
      try {
        const result = await EntityApiService.generatePersonelZimmetBilgiFisi(personelId);
        if (!result) {
          throw new Error("API'den geçerli bir yanıt alınamadı (generatePersonelZimmetBilgiFisi).");
        }

        if (showSuccessToast) {
          toast.success(`${result.personelBilgileri?.ad || 'Personel'} zimmet bilgi fişi başarıyla oluşturuldu. (${result.malzemeSayisi || 0} malzeme)`);
        }

        // Tutanak listesini güncelle
        set(state => ({
          datas: [result.tutanak, ...state.datas].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
          loadingAction: false,
          currentData: result.tutanak,
        }));

        return result;
      } catch (error) {
        const message = error?.response?.data?.message || error.message || `Personel zimmet bilgi fişi oluşturma başarısız.`;
        toast.error(`Zimmet bilgi fişi oluşturma hatası: ${message}`);
        set({ error: message, loadingAction: false });
        return null;
      }
    },
