  ...
  return (
    <ScrollArea className="h-[calc(100vh-230px)]">
      <div className="p-2 md:p-4">
        ...
        {itemData.burolar && itemData.burolar.length > 0 && (
          <DetailSection title={`Bağlı Bürolar (${itemData.burolar.length})`}>
            <ul className="list-disc list-inside pl-4 space-y-1 text-sm">
              {itemData.burolar.slice(0, 5).map(buro => (
                <li key={buro.id} className="flex items-center justify-between">
                  <span>{buro.ad}</span>
                  {buro.amir && (
              ...
              {itemData.burolar.length > 5 && <li className="text-muted-foreground text-xs">... ve {itemData.burolar.length - 5} büro daha</li>}
            </ul>
          </DetailSection>
        )}
        
        {itemData.malzemeler && itemData.malzemeler.length > 0 && (
          <DetailSection title={`İş Karşılığı Malzemeler (${itemData.malzemeler.length})`}>
            <ul className="list-disc list-inside pl-4 space-y-1 text-sm">
...
export const Sube_DetailSheet = props => {
  return (
