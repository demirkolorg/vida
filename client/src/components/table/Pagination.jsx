...
export function DataTablePagination({
  ...
  const actualTotalRows = typeof totalRowCount === 'number' ? totalRowCount : clientSideTotalRows;

  const firstRowIndex = actualTotalRows > 0 ? pageIndex * pageSize + 1 : 0;
  const lastRowIndexOnPage = Math.min((pageIndex + 1) * pageSize, actualTotalRows);
  ...
  return (
    <div className="flex items-center justify-between px-2 py-3"> {/* py-3 eklendi */}
      ...
      <div className="flex items-center space-x-6 lg:space-x-8">
        <div className="flex items-center space-x-2">
          <p className="text-sm font-medium">Satır:</p> {/* "Sayfa başına satır" yerine sadece "Satır:" */}
          <Select
            value={`${pageSize}`}
            onValueChange={value => {
          ...
          >
            <SelectTrigger className="h-8 w-[80px]"> {/* w-[80px] yerine w-[75px] */}
              <SelectValue placeholder={pageSize} />
            </SelectTrigger>
            <SelectContent side="top">
              {[10, 20, 50, 100, 500, 1000, 5000].map(size => (
        ...
        <div className="flex w-auto items-center justify-center text-sm font-medium"> {/* w-[100px] kaldırıldı, w-auto yapıldı */}
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button
            variant="outline"
            className="hidden h-8 w-8 p-0 lg:flex"
            onClick={() => table.setPageIndex(pageCount > 0 ? pageCount - 1 : 0)} // pageCount > 0 kontrolü eklendi
            disabled={!table.getCanNextPage()}
          >
