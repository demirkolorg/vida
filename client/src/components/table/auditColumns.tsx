...
export const getAuditColumns = <TData extends AuditableBase>(): ColumnDef<TData, any>[] => [
      ...
      },
  {
    ...
    cell: ({ row }) => {
      const createdByPersonel = row.original.createdBy as PersonelReference | undefined | null; // Tip ataması
    ...
    accessorFn: (row) => (row.createdBy as PersonelReference | undefined | null)?.ad, // Filtreleme ve sıralama için
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const updatedByPersonel = row.original.updatedBy as PersonelReference | undefined | null; // Tip ataması
    ...
    accessorFn: (row) => (row.updatedBy as PersonelReference | undefined | null)?.ad, // Filtreleme ve sıralama için
    filterFn: turkishCaseInsensitiveFilterFn,
    size: 200,
    enableHiding: true, // Varsayılan olarak gizlenebilir olsun
  },
